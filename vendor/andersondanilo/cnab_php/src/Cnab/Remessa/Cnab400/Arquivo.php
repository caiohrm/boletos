...
class Arquivo implements \Cnab\Remessa\IArquivo
{
    ...
    private function setDados(array $campos,array $params)
    {
    ...
    public function configureBanrisul(array $params)
    {
        $campos = array('codigo_banco','nome_empresa','data_gravacao');
        $this->setDados($campos,$params);
        $this->data_gravacao = $this->configuracao['data_gravacao'];
        //$this->header->codigo_banco = $this->banco['codigo_do_banco'];
        //$this->header->nome_banco = $this->banco['nome_do_banco'];
        $this->header = new Header($this);
        $this->header->nome_empresa = $this->configuracao['nome_empresa'];
        $this->header->constante = '01REMESSA';
        $this->header->constante_2= '041BANRISUL';
        $this->header->tipo_proces = 'X';
        $this->header->constante_1='000001';
        $this->header->brancos_1='';
        $this->header->brancos_2='';
        $this->header->brancos_3='';
        $this->header->brancos_4='';
        $this->header->brancos_5='';
        $this->header->brancos_6='';
        $this->header->brancos_7='';
        $this->header->codigo_banco = $params['codigo_banco'];
        $this->header->data_gravacao = $params['data_gravacao']->format('dmy');
        $this->header->codigo_servico='';
        $this->header->codigo_cliente = '';

    }

    private function detalheBanrisul(array $boleto, $tipo = 'remessa')
    {
        $detalhe = new Detalhe($this);
        //$detalhe->constante = '1';
        $detalhe->brancos= '';
        $detalhe->cedente = $boleto['cedente'];
        $detalhe->brancos2 = '';
        $detalhe->ident_titulo = $boleto['ident_titulo'];
        $detalhe->nosso_numero = $boleto['nosso_numero'];
        $detalhe->mensagem = '';
        $detalhe->brancos3 = '';
        $detalhe->carteira = '1';
        $detalhe->ocorrencia = '1';
        $detalhe->seu_numero = $boleto['seu_numero'];
        $detalhe->vencimento = $boleto['vencimento']->format('dmy');
        $detalhe->valor = $boleto['valor'];
        $detalhe->constante2 = '041';
        $detalhe->brancos4 = '';
        $detalhe->tipo = '';
        $detalhe->aceite = 'A';
        $detalhe->emissao = '';
        ...
        $detalhe->iof = '';
        $detalhe->abatimento = '';
        $detalhe->codigo_inscricao = $boleto['codigo_inscricao'];
        $detalhe->inscricao = $boleto['inscricao'];
        $detalhe->nome = $boleto['nome'];
        $detalhe->brancos5 = '';
        $detalhe->endereco = $boleto['endereco'];
        $detalhe->brancos6 = '';
        $detalhe->taxamulta = '';
        $detalhe->diasmulta = '';
        $detalhe->cep = $boleto['cep'];
        $detalhe->cidade = $boleto['cidade'];
        $detalhe->uf = $boleto['uf'];
        $detalhe->antecipado = '';
        $detalhe->brancos7 = '';
        $detalhe->valdesconto = '';
        $detalhe->protesto = '';
        $detalhe->brancos8 = '';
        //$detalhe->numero_sequencial = $boleto['numero_sequencial'];
        $this->detalhes[] =$detalhe;
    }

    public function insertDetalhe(array $boleto, $tipo = 'remessa')
    {
        $this->detalheBanrisul($boleto);
        return;
        ...
        if ($tipo == 'remessa') {
            ...
            $detalhe->codigo_inscricao = 1;
            $detalhe->inscricao = $this->prepareText($this->configuracao['inscricao'], '.-/');
